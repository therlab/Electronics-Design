 ...
 * @param showPrefix: 是否显示"0x"前缀 (1显示/0不显示)
 */
void OLED_ShowHex(uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t sizey, uint8_t showPrefix)
{
    uint8_t t, temp;
    uint8_t m = (sizey == 8) ? 2 : 0;  // 8x16字体间距调整
    
    // 显示"0x"前缀
    if(showPrefix) {
        OLED_ShowString(x, y, (uint8_t *)"0x", sizey);
        x += (sizey == 8) ? 12 : 16;  // 调整x坐标
    }
    
    // 显示十六进制数字
    for(t = 0; t < len; t++) {
        temp = (num >> ((len - t - 1) * 4)) & 0x0F;  // 获取4位十六进制值
        
        // 转换为ASCII字符
        if(temp < 10) {
            OLED_ShowChar(x + (sizey/2 + m)*t, y, temp + '0', sizey);
        } else {
            OLED_ShowChar(x + (sizey/2 + m)*t, y, temp - 10 + 'A', sizey);
        }
    }
}
